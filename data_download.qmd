...
cleaned_eight$DAMAGE_CROPS = as.numeric(cleaned_eight$DAMAGE_CROPS)
```

```{r}
#change M values in damage property to K
cleaned_eight$DAMAGE_PROPERTY = str_replace(cleaned_eight$DAMAGE_PROPERTY,  "(\\d+(\\.\\d+)?)M",  function(x) {
  number = as.numeric(str_extract(x, "\\d+(\\.\\d+)?"))
 paste0(number * 10000, ".00K") })

cleaned_eight = cleaned_eight %>%
  mutate(DAMAGE_PROPERTY = str_remove(DAMAGE_PROPERTY, "K"))

cleaned_eight$DAMAGE_PROPERTY = as.numeric(cleaned_eight$DAMAGE_PROPERTY)
